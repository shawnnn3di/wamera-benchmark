...
def previewbatch(imgbatch, real, fake, box=None, n=4):

    # imgbatch = imgbatch.detach().cpu().numpy()
    # real = [_.detach().float().cpu().numpy() for _ in real]
    fake = [_.detach().float().cpu().numpy() for _ in fake]
    ret = list()
    pcks = np.zeros((1, 20))
    
    # fake = [_.transpose([0, 2, 1, 3, 4]) for _ in fake]
    # real = [_.transpose([0, 2, 1, 3, 4]) for _ in real]
    for i in range(min(n, len(imgbatch))):
        ...
        candidate_r, subset_r = _preview(canvas, real[1].squeeze()[i], real[0].squeeze()[i])
        ...
        candidate_f, subset_f = _preview(canvas, fake[1].squeeze()[i], fake[0].squeeze()[i])
        canvas = util.draw_bodypose(np.ascontiguousarray(canvas.transpose(1, 2, 0)), candidate_f, subset_f)
        ret.append(canvas)
        # print('elapsed: %.2f' % (time.time() - t1))
        
        if box is not None:
            pcks += pck(subset_r, subset_f, candidate_r, candidate_f, box[i])

    return np.stack(ret)[:, :, :, [2, 1, 0]], pcks / n if box is not None else None
...
def _preview(oriImg, Mconv7_stage6_L1, Mconv7_stage6_L2):
    ...
    for k in range(len(mapIdx)):
        ...
        if (nA != 0 and nB != 0):
            ...
            # if len(connection_candidate) > 100000:
            #     break
...
def preview(oriImg, Mconv7_stage6_L1, Mconv7_stage6_L2):
    scale_search = [1.]
    boxsize = 368
    stride = 8
    padValue = 128
    ...
    for m in range(len(multiplier)):
        scale = multiplier[m]
        imageToTest = cv2.resize(oriImg, (0, 0), fx=scale, fy=scale, interpolation=cv2.INTER_CUBIC)
        # imageToTest = np.zeros((3, 368, 368))
        imageToTest_padded, pad = util.padRightDownCorner(imageToTest, stride, padValue)
        ...
        heatmap = np.transpose(np.squeeze(Mconv7_stage6_L2), (1, 2, 0)).astype(float)  # output 1 is heatmaps
        heatmap = cv2.resize(heatmap, (0, 0), fx=stride, fy=stride, interpolation=cv2.INTER_CUBIC)
        heatmap = heatmap[:imageToTest_padded.shape[0] - pad[2], :imageToTest_padded.shape[1] - pad[3], :]
        heatmap = cv2.resize(heatmap, (oriImg.shape[1], oriImg.shape[2]), interpolation=cv2.INTER_CUBIC)

        # paf = np.transpose(np.squeeze(net.blobs[output_blobs.keys()[0]].data), (1, 2, 0))  # output 0 is PAFs
        paf = np.transpose(np.squeeze(Mconv7_stage6_L1), (1, 2, 0)).astype(float)  # output 0 is PAFs
        paf = cv2.resize(paf, (0, 0), fx=stride, fy=stride, interpolation=cv2.INTER_CUBIC)
        paf = paf[:imageToTest_padded.shape[0] - pad[2], :imageToTest_padded.shape[1] - pad[3], :]
        paf = cv2.resize(paf, (oriImg.shape[1], oriImg.shape[2]), interpolation=cv2.INTER_CUBIC)
