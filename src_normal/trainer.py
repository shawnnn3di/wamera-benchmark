...
from modeling import student_0, student_1, combine_translator
from previewer import previewbatch
from utils import Recorder, add_args, matthewloss, l2loss
...
class trainer:
    ...
    def iterate(self, train=True, epoch=-1):
        ...
        # for i, batch in pbar:
            ...
            loss_sm = self.criterion_fn(sm, self.halving(batch['sm'].float()))
            loss_jhm = self.criterion_fn(jhm, self.halving(torch.tensor(batch['jhm'], dtype=jhm.dtype, device=jhm.device)))
            loss_paf = self.criterion_fn(paf, self.halving(torch.tensor(batch['paf'], dtype=jhm.dtype, device=jhm.device)), k=1, b=0.3)
